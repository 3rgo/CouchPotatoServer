    ...
    def _search(self, media, quality, results):
        ...
        for title in media['info']['titles']:
            try:
                ...
                url = self.urls[
                          'search'] + 'cat[]=MOVIE|DVDR&cat[]=MOVIE|DVDRIP&cat[]=MOVIE|BDRIP&cat[]=MOVIE|VOSTFR&cat[]=MOVIE|HD|720p&cat[]=MOVIE|HD|1080p&cat[]=MOVIE|REMUXBR&cat[]=MOVIE|FULLBR&cat[]=ANIME&' + urllib.urlencode(
                    {'search': unicodedata.normalize('NFKD', title).encode('ASCII', 'ignore').replace('\'',
                                                                                                      ' ')}) + '&order=Time&way=desc'
                        ...
                        if torrentTable:
                            ...
                            for torrentRow in torrents:
                                ...
                                if len(splittedReleaseName) > 1:
                                    ...
                                    nameSplit = ''.join(match.findall(
                                        unicodedata.normalize('NFKD', cleanedReleaseName).encode('ASCII', 'ignore')))
                                    titleSplit = ''.join(match.findall(
                                        unicodedata.normalize('NFKD', title.upper()).encode('ASCII', 'ignore')))
